...
impl ImtrPreview{
    ...
    fn update_pixbuf(&self, e: ImtrEventObject){
        ...
        *self.imp().path_a.borrow_mut() = Some(path_a.clone());
        *self.imp().path_b.borrow_mut() = Some(path_b.clone());
    ...
    pub fn get_path(&self, dec: Decision) -> Option<PathBuf>{
        match dec{
            Decision::Left  => self.imp().path_a.borrow().clone(),
            Decision::Right => self.imp().path_b.borrow().clone(),
            Decision::Undef => None,
        }
    }
    // new /////////////////////////////////////////////////
